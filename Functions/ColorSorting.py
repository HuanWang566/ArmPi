        ...
        if __isRunning:
            for i in range(camera_number):     
                if detect_color[i] != 'None' and start_pick_up[i]:  #如果检测到方块没有移动一段时间后，开始夹取
                    ...
                    if result == False:
                        print('The robotic arm can not reach the target place!')
                    else:
...
class Detector:
    ...
    def bi_camera_coodinate_correcte(x, y, index):
        if index == 0:
            x -= 14
            y -= 13
        else:
    ...
    def run(self, img):
        ...
        if get_roi[self.index] and not start_pick_up[self.index]:
            get_roi[self.index] = False
        ...
        if not start_pick_up[self.index]:
                ...
                if not start_pick_up[self.index]:
                    ...
                    if distance < 0.5:
                        ...
                        if time.time() - self.t1 > 1:
                            rotation_angle[self.index] = self.rect[2] 
                            self.start_count_t1 = True
                            world_X[self.index], world_Y[self.index] = np.mean(np.array(self.center_list).reshape(self.count, 2), axis=0)
                            world_X[self.index], world_Y[self.index] = bi_camera_coodinate_correcte(world_X[self.index], world_Y[self.index], self.index)
                            self.center_list = []
                            self.count = 0
                            start_pick_up[self.index] = True
                    else:
                        self.t1 = time.time()
                        self.start_count_t1 = True
                        self.center_list = []
                    ...
                    if len(self.color_list) == 3:  #多次判断
                        ...
                        self.color_list = []
                        if color == 1:
                            detect_color[self.index] = 'red'
                            self.draw_color = range_rgb["red"]
                        elif color == 2:
                            detect_color[self.index] = 'green'
                            self.draw_color = range_rgb["green"]
                        elif color == 3:
                            detect_color[self.index] = 'blue'
                            self.draw_color = range_rgb["blue"]
                        else:
                            detect_color[self.index] = 'None'
                            self.draw_color = range_rgb["black"]
            else:
                if not start_pick_up[self.index]:
        ...
        cv2.putText(img, "Color: " + detect_color[self.index], (10, img.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.65, self.draw_color, 2)
        return img
...
# def camera_detect:
#     my_camera = Camera.Camera()
#     # raspberryPi has at most 4 USB interfaces
#     for i in range(4):
